...
public class CorrespondenceViewAuthorization {
  ...
  public void assertCanView(AppUserEntity viewer, CorrespondenceEntity correspondence) {
    ...
    if (departmentAccessChecker.isSameDepartmentAsOwner(viewer, correspondence)) {
      log.debug(
          "CorrespondenceEntity view allowed (same department as owner): userId={} correspondenceId={}",
          userId,
          correspondenceId);
      return;
    }
    ...
    if (workflowParticipationChecker.isParticipating(userId, referenceNumber)) {
      log.debug(
          "CorrespondenceEntity view allowed (workflow): userId={} correspondenceId={}",
          userId,
          correspondenceId);
      return;
    }
    ...
    if (privilegedRoleChecker.hasPrivilegedViewRole(userId)) {
      log.debug(
          "CorrespondenceEntity view allowed (privileged role): userId={} correspondenceId={}",
          userId,
          correspondenceId);
      return;
    }
    ...
    log.warn(
        "CorrespondenceEntity view denied: userId={} correspondenceId={} referenceNumber={} (not same department as owner, not workflow participant, no privileged view role)",
        userId,
        correspondenceId,
        referenceNumber);
