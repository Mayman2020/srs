  ...
  public UserCapabilitiesDto loadForCurrentUser() {
    ...
    List<CapabilityScreenDto> screens =
        uiScreenRepository.findByDeletedAtIsNullOrderBySortOrderAsc().stream()
            ...
            .sorted(Comparator.comparing(UiScreenEntity::getSortOrder, Comparator.nullsLast(Integer::compareTo)))
            .map(UserCapabilitiesMapper::toCapabilityScreen)
            .toList();

    return UserCapabilitiesMapper.toCapabilitiesDto(List.copyOf(roleCodes), permissionCodes, screens);
  }
}
