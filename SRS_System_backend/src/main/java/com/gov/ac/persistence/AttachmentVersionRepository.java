...
public interface AttachmentVersionRepository extends JpaRepository<AttachmentVersion, Long> {
  ...
  @Query("select v from AttachmentVersion v where v.id = :id and v.deletedAt is null")
  Optional<AttachmentVersion> findByIdAndDeletedAtIsNullWithAttachment(@Param("id") Long id);

  @Query(
      "select v from AttachmentVersion v join fetch v.attachment att "
          + "where att.id in :attachmentIds and v.deletedAt is null "
          + "order by att.id asc, v.versionNumber asc")
  List<AttachmentVersion> findAllForDetailByAttachmentIdIn(
